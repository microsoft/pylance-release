...
```python
def _unused_helper(x: int) -> int:  # Defined but never called
    return x * 2
...
**Fix — remove the unused function or use it:**
...
Or, if the function is used elsewhere, make sure it’s actually called or exported:
