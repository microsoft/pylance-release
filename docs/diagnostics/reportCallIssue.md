...
add(1)        # Error: Argument missing for parameter "b"
add(1, 2, 3)  # Error: Too many positional arguments
```

**Fix — pass the correct number and type of arguments:**

```python
add(1, 2)  # OK
```

**Example — calling a non-callable:**

```python
x: int = 42
x()  # Error: Object of type "int" is not callable
```
